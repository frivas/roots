...
records immutable previous deploys as rollback targets, runs the canary and
runtime SLO checks, and retains the evidence for 30 days. Failed or cancelled
CI gets an explicit rejected release-gate run.

The workflow is read-only. Provider settings must keep `main` protected and
must hold domain promotion behind deployment checks if the account supports
that feature. Without staged promotion, this is a post-deploy verification
gate and cannot prevent a provider from briefly serving a bad build.
...
A rollback is authorized separately from validation. Use the immutable rollback
IDs in `deployment-discovery.json`; never use a branch name or mutable alias.
After both providers report ready, run `Deployed canary` for the rollback SHA.
An alias change alone is not evidence of recovery.
